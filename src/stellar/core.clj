...
(def statistic-data
  {:inflows {:total-coins 0, :total-txns 0, :avg-txn 0, :txn-frqcy {}}
   :outflows {:total-coins 0, :total-txns 0, :avg-txn 0}
   :max-bal 0
   :max-time 0
   :min-bal 0
   :min-time 0
...
(defn get-all-acct-dest-in-range [start-range end-range]
  (dissoc
   (reduce add-keys-to-map {} (keep #(get-ledger-destinations %)
                                    (vec (range start-range end-range))))
   nil))
...
(defn process-all-flow-data [txn-vec acct-id]
  (let [filtered-vec (filter-txn-by-payment txn-vec "Payment")]
    (loop[flow-stats statistic-data cnt 0 cur-bal 0 cur-bal-sum 0 max-time ((nth filtered-vec 0) :date) min-time ((nth filtered-vec 0) :date) max-bal 0 min-bal 0]
      (if (= cnt (dec(count filtered-vec)))
        (->
         ...
         (assoc
           ...
           :avg-balance (format "%.0f"(double(/ cur-bal-sum (+ (get-in flow-stats [:inflows :total-txns])(get-in flow-stats [:outflows :total-txns])))))
...
(def fmap {5000 2, 3000 3, 1000 1})

(-> (update-index-map fmap 5000)
    (update-index-map 5000)
    (update-index-map 2000))

(def temp-stat-map statistic-data)

(def test-stat-map (->(assoc-in temp-stat-map [:inflows :txn-frqcy] fmap)
                      (get :inflows)
                      (update-stat-map 5000)
                      (update-stat-map 3000)))


(def tempvec (get-all-txn-data-vec "gHuAWt6ZQ5jskU7TgyQ8B1c7wH1LRfzyb6"))
(def tempvec2 (get-all-txn-data-vec "gHuAWt6ZQ5jskU7TgyQ8B1c7wH1LRfzyb6"))
(keys (nth tempvec 3))

(def testvec(filter-txn-by-payment tempvec2 "Payment"))
(get-txn-vec "gHuAWt6ZQ5jskU7TgyQ8B1c7wH1LRfzyb6")

testvec

(process-all-flow-data tempvec2 "gHuAWt6ZQ5jskU7TgyQ8B1c7wH1LRfzyb6")
